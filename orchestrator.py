...
def run_full_pipeline(
    ...
    All architect agents receive RAG context (retrieved reference patterns
    + web research) so every design decision is grounded in real-world patterns.
    """
    ...
    system_rules = safe_read_file(
        "system/SYSTEM.md",
        "Follow best practices for data engineering, analytics, and architecture."
    )
    skill_pipeline = safe_read_file(
        "skills/good_pipeline_design.md",
        "Design robust, scalable, and production-ready data pipelines."
    )
    skill_model = safe_read_file(
        "skills/good_data_model.md",
        "Design practical analytical data models with clear grains and keys."
    )
    ...
    # =========================================================
    log("Running Dataset Profiler Agent...")
    ...
    if HAS_DATASET_VIZ:
        try:
            log("Running Dataset Visualization Agent...")
            ...
            log(f"Dataset Visualization Agent skipped: {e}")

    # =========================================================
    # RAG CONTEXT — built once, reused by all architect agents
    # =========================================================
    log("Building RAG context (domain detection + pattern retrieval + web search)...")
    try:
        ...
        write_output("rag_context.md", rag_context)
        log(f"RAG context built. Detected domain: {detected_domain}")
    except Exception as e:
        log(f"RAG context build failed ({e}), continuing without RAG.")
        rag_context = ""
    ...
    # =========================================================
    log("Running Schema & Data Contracts Agent...")
...
"""
    ...
    # =========================================================
    log("Running Requirements Interpreter Agent...")
...
Extract business goals, key metrics, core entities, data sources,
granularity, assumptions, and open questions."""
    )
...
Use the retrieved reference architecture patterns above to inform your analysis.
"""
    ...
    # =========================================================
    log("Running Ingestion Strategy Agent...")
    ...
    # =========================================================
    log("Running Storage Layout Agent...")
    ...
    # =========================================================
    log("Running Orchestration Agent...")
    ...
    # =========================================================
    log("Running Security & Governance Agent...")
    ...
    # =========================================================
    log("Running Pipeline Architect Agent...")
    pipeline_agent_text = safe_read_file(
        "agents/pipeline_architect.txt",
        """You are the Pipeline Architect agent.
Design the end-to-end data pipeline architecture including sources, ingestion,
processing, storage, orchestration, serving, and monitoring."""
    )
...
Reference the retrieved architecture patterns above when making design decisions.
"""
    ...
    # =========================================================
    log("Running Data Modeler Agent...")
    data_modeler_text = safe_read_file(
        "agents/data_modeler.txt",
        """You are the Data Modeler agent.
Design the analytical data model, fact tables, dimension tables, grains,
keys, relationships, and assumptions."""
    )
...
Reference the retrieved architecture patterns above for modelling conventions.
"""
    ...
    # =========================================================
    log("Running Data Quality Engineer Agent...")
    dq_agent_text = safe_read_file(
        "agents/data_quality_engineer.txt",
        """You are the Data Quality Engineer agent.
Define data quality checks, SLAs, freshness expectations, critical
tables/columns, monitoring, alerting, failure handling, and risks."""
    )
...
"""
    ...
    # =========================================================
    log("Running Performance & Cost Optimization Agent...")
    perf_agent_text = safe_read_file(
        "agents/performance_cost_optimizer.txt",
        """You are the Performance & Cost Optimization agent.
Review performance bottlenecks, storage and compute optimizations,
query efficiency, cost strategies, tradeoffs, and final recommendations."""
    )
...
"""
    ...
    # =========================================================
    log("Running Analytics / BI Agent...")
    analytics_agent = AnalyticsBIAgent(
        context=context,
        rag_context=rag_context
    )
    ...
    # =========================================================
    log("Running Mermaid AI Diagram Agent...")
    ...
    # =========================================================
    log("Running Documentation Writer Agent...")
    doc_agent_text = safe_read_file(
        "agents/documentation_writer.txt",
        """You are the Documentation & Architecture Writer agent.
Produce a clean, professional README-style document explaining the system,
architecture, design decisions, workflow, and future improvements."""
    )
...
"""
    ...
    # =========================================================
    log("Running Rule-Based Validator Agent...")
    ...
    # =========================================================
    log("Running Cost Estimation Agent...")

    # Detect complexity from rag_context for accurate cost multiplier
    rag_lower = rag_context.lower()
    ingestion_lower = context.get("ingestion_strategy", "").lower()
    combined = rag_lower + " " + ingestion_lower

    if any(k in combined for k in ["very high", "data mesh", "sox", "hipaa", "pci"]):
        complexity = "very high"
    elif any(k in combined for k in ["streaming", "cdc", "financial risk", "healthcare"]):
        complexity = "high"
    elif any(k in combined for k in ["medallion", "lakehouse", "iceberg", "delta"]):
        complexity = "medium-high"
    elif any(k in combined for k in ["dbt", "airflow", "redshift", "snowflake"]):
        complexity = "medium"
    else:
    ...
    log(f"Cost Estimation Agent completed. Estimated: ${cost_result.get('total_monthly', 0):.2f}/month")
